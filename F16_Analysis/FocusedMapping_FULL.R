...
AddAndImpTest<-function(locus1,locus2,ImprintingScores,AdditiveScores,Phenotypes,phenOfInterest,DEgene,ASEgene){
	...
	### Run ANOVA model and return P-value's 
	
	#print(summary( aov(phen~ L1_Imp*L2_Imp+L1_Add*L2_Imp+L2_Add*L1_Imp+L1_Add*L2_Add,data.frame(datMat)) ))
	
	c( summary( aov(phen~ L1_Imp*L2_Imp+L1_Add*L2_Imp+L2_Add*L1_Imp+L1_Add*L2_Add,data.frame(datMat)) )[[1]][["Pr(>F)"]][1:8],DEgene,ASEgene,phenOfInterest,ImprintingScores[locus1,4],ImprintingScores[locus2,4]  )
...
colnames(EpistasisData)<-c("DE_Imp","ASE_Imp","DE_Add","ASE_Add","DE_Imp:ASE_Imp","DE_Add:ASE_Imp","DE_Imp:ASE_Add","DE_Add:ASE_Add","DE","ASE","Phenotype","DE Marker","ASE Marker")
...
QObj.DE.Imp<-qvalue(as.numeric(as.character(EpistasisData$DE_Imp)))
QObj.ASE.Imp<-qvalue(as.numeric(as.character(EpistasisData$ASE_Imp)))


QObj.DE.Add<-qvalue(as.numeric(as.character(EpistasisData$DE_Add)))
QObj.ASE.Add<-qvalue(as.numeric(as.character(EpistasisData$ASE_Add)))


QObj.DE.Imp.ASE.Imp<-qvalue(as.numeric(as.character(EpistasisData$"DE_Imp:ASE_Imp")))

QObj.DE.Add.ASE.Imp<-qvalue(as.numeric(as.character(EpistasisData$"DE_Add:ASE_Imp")))

QObj.DE.Imp.ASE.Add<-qvalue(as.numeric(as.character(EpistasisData$"DE_Imp:ASE_Add")))

QObj.DE.Add.ASE.Add<-qvalue(as.numeric(as.character(EpistasisData$"DE_Add:ASE_Add")))



hist(QObj.DE.Imp.ASE.Imp)
plot(QObj.DE.Add.ASE.Add)


sum(QObj.DE.Imp$qvalues<=0.1)
sum(QObj.ASE.Imp$qvalues<=0.1)

sum(QObj.DE.Add$qvalues<=0.1)
sum(QObj.ASE.Add$qvalues<=0.1)


sum(QObj.DE.Imp.ASE.Imp $qvalues<=0.1)

sum(QObj.DE.Add.ASE.Imp $qvalues<=0.1)

sum(QObj.DE.Imp.ASE.Add $qvalues<=0.1)

sum(QObj.DE.Add.ASE.Add $qvalues<=0.1)
